...
function getDateTimeString(date) {
  const typeStringOfParam = getTypeString(date); // console.log('getDateTimeString() : Type of date parameter is', typeStringOfParam);
...
function insertNumberIntoArray(n, array) {
  ...
  let result = clone(array); // Array.splice modifies the array in place. See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/splice

  result.splice(i, 0, n);
  return result;
}
function insertionSort(array) {
  return array.reduce((accumulator, n) => insertNumberIntoArray(n, accumulator), []);
}
...
function safeJsonParse(str, dflt = undefined) {
  try {
...
function isArrayOfNumbers(arg) {
  return isArray(arg) && arg.every(element => getTypeString(element) === '[object Number]');
}
function sum(arg) {
  ...
  return arg.reduce((accumulator, n) => accumulator + n, 0);
}
function mean(arg) {
...
function mode(arg) {
  ...
  return Object.keys(hist).reduce((accumulator, element) => {
